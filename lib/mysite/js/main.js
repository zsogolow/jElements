_(document).bind('DOMContentLoaded', function () {
    ...
    function init() {
        ...
        setTimeout(function () {
            ...
            _('.tiltLabel').appear(_bodyMain, function (element) {
                _(element).toggleClass('tilt')
            }, 10);
            _('.slideLabel').appear(_bodyMain, function (element) {
                _(element).toggleClass('slide-in')
            }, 10);
            _('.fadeLabel').appear(_bodyMain, function (element) {
                _(element).toggleClass('fade-in')
            }, 10);
        }, 2500);
    }
    ...
    _bodyMain.bind('wheel', function (event) {
        if (isJacked == true) {
            if (notScrolling) {
                if (!lastDirection || lastDirection == event.deltaY) {} else {
                    wheelCount = 0;
                }
                ...
                if (wheelCount % 3 == 0) {
                    ...
                    if (_newSection) {
                        wheelCount = 0;
                        notScrolling = false;
                        smoothScroll.scrollToAnchor(_newSection.item(0), {
                            speed: 800
                        }, function () {
                            notScrolling = true;
                        });
    ...
    _moreButtons.bind('click', function (evt) {
        ...
        smoothScroll.scrollToAnchor(_('.body-main section[data-index="' + (parseInt(index) + 1) + '"]').item(0), {
            speed: 500
        });
    });

    _('.navBubble').bind('click', function (evt) {
        var index = _(this).data('index');
        smoothScroll.scrollToAnchor(_('.body-main section[data-index="' + parseInt(index) + '"]').item(0), {
            speed: 500
        });
    });
    ...
    _lis.bind('click', function (event) {
        ...
        smoothScroll.scrollToAnchor(_('#' + section).item(0), {
            speed: 800
        }, function () {});
    });
